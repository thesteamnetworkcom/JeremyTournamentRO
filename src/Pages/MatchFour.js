...
const ConnectedMatchFour = (props) => {
	const { classes } = props;
				...
				<div className={classes.leftSide}>
					{Object.keys(props.matches).length === 0 ? "" :
						props.matches.matchFour.Matches.map((el,index)=>
							index < 2 ?
								<MatchLargeV2 key={index} match={el} index={index} group="matchFour" /> : ''
						)
					}
				</div>
...
const MatchFour = connect(mapStateToProps)(ConnectedMatchFour);

export default withStyles(styles)(MatchFour);